#!/usr/bin/env python3
# add_product_images_pixabay.py - Script para agregar imágenes usando Pixabay API
...
# Pixabay API - Sin API key (limitado pero funciona)
# Obtener API key gratis en: https://pixabay.com/api/docs/

# Mapeo de categorías a términos de búsqueda de Pixabay
CATEGORY_KEYWORDS = {
...
def get_pixabay_image_url(keyword, width=400, height=400):
    """Obtener URL de imagen de Pixabay por keyword"""
    try:
        # Usar Pixabay API sin key (limitado pero funciona)
        ...
        print(f"Error con Pixabay API: {e}")
        # Fallback a placeholder
        return f"https://via.placeholder.com/{width}x{height}/4ECDC4/FFFFFF?text={keyword.replace(' ', '+')}"

def generate_image_urls(category_name, count=3):
    """Generar URLs de imágenes para una categoría usando Pixabay"""
    keyword = get_category_keyword(category_name)
    
    # Imagen principal
    main_image = get_pixabay_image_url(keyword, 400, 400)
    ...
    for i in range(count - 1):
        ...
        img_url = get_pixabay_image_url(alt_keyword, 400, 400)
...
def main():
    ...
    with app.app_context():
        print("🖼️  Agregando imágenes relevantes usando Pixabay...")
        ...
        for product in products:
            try:
                ...
                updated_count += 1
                print(f"✅ {product.name} - {category_name} - Imágenes Pixabay agregadas")
        ...
        try:
            db.session.commit()
            print(f"\n🎉 ¡Éxito! {updated_count} productos actualizados con imágenes de Pixabay")
