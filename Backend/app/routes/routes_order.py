...
@jwt_required()
def get_orders():
    """Get all user orders with their items"""
    ...
    # Serialize orders with their items
    orders_data = []
...
def create_order():
    ...
    if not data.get('address_id'):
        return jsonify({'message': 'address_id is required'}), 400
...
def update_order_status(order_id):
    ...
    if data['status'] not in valid_statuses:
        return jsonify({'message': f'Invalid status. Valid statuses: {", ".join(valid_statuses)}'}), 400
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'message': 'Error updating order status'}), 400
...
def cancel_order(order_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'message': 'Error cancelling order'}), 400
